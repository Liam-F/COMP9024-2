...
public class AVLTree<K, V> extends BinarySearchTree<K, V> implements Dictionary<K, V>
{
    ...
    {
        return new AVLNode<K, V>(element, parent, left, right); // now use AVL
                                                                // nodes
    }
    ...
    protected void rebalance(Position<Entry<K, V>> zPos)
    {
        ...
        { // traverse up the tree towards the root
            ...
            if (!isBalanced(zPos))
            {
                ...
                zPos = restructure(xPos); // tri-node restructure (from parent
                                          // class)
    ...
    {
        Entry<K, V> toReturn = super.insert(k, v); // calls our createNode
                                                   // method

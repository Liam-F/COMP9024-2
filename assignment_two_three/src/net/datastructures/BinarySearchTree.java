...
public class BinarySearchTree<K, V> extends LinkedBinaryTree<Entry<K, V>> implements Dictionary<K, V>
{
    ...
    protected Position<Entry<K, V>> actionPos; // insert node or removed node's
                                               // parent
    ...
    protected void addAll(PositionList<Entry<K, V>> L, Position<Entry<K, V>> v, K k)
    {
        ...
        if (!isExternal(pos))
        { // we found an entry with key equal to k
            addAll(L, left(pos), k);
    ...
    public Entry<K, V> remove(Entry<K, V> ent) throws InvalidEntryException
    {
        ...
        { // entry is at a node with internal children
            Position<Entry<K, V>> swapPos = remPos; // find node for moving
                                                    // entry
    ...
    protected Position<Entry<K, V>> restructure(Position<Entry<K, V>> x)
    {
        ...
        BTPosition<Entry<K, V>> xx = (BTPosition<Entry<K, V>>) x, yy = (BTPosition<Entry<K, V>>) y,
                zz = (BTPosition<Entry<K, V>>) z;
        if (xLeft && yLeft)
        {
            a = xx;
            b = yy;
